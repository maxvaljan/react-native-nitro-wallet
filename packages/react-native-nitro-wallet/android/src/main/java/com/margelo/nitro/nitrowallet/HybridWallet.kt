...
final class HybridWallet : HybridWalletSpec() {
    ...
    override fun getCapabilities(): Promise<WalletCapabilities> = Promise.async {
    ...
    override fun canAddPasses(provider: WalletProvider): Promise<Boolean> = when (provider) {
    ...
    override fun addPkPassFromUrl(options: AddPkPassFromUrlOptions): Promise<AddPassResult> =
        Promise.rejected(WalletException("Apple .pkpass files are only supported on iOS."))

    override fun addPkPassFromBase64(options: AddPkPassFromBase64Options): Promise<AddPassResult> =
        Promise.rejected(WalletException("Apple .pkpass files are only supported on iOS."))

    override fun saveGoogleWalletPass(options: SaveGoogleWalletPassOptions): Promise<SaveGoogleWalletPassResult> {
        val context = NitroModules.applicationContext
            ?: return Promise.rejected(
                WalletException("No React Native application context is available.")
            )
    ...
    override fun hasPass(identifier: WalletPassIdentifier): Promise<Boolean> =
        Promise.rejected(WalletException("Local pass lookup is only supported on iOS."))

    override fun openPass(identifier: WalletPassIdentifier): Promise<Boolean> =
        Promise.rejected(WalletException("Opening local wallet passes is only supported on iOS."))

    override fun removePass(identifier: WalletPassIdentifier): Promise<Boolean> =
        Promise.rejected(WalletException("Removing local wallet passes is only supported on iOS."))
